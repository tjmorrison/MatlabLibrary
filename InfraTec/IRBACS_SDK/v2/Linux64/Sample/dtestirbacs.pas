...
type
  ...
  TfrmTestLib = class(TForm)
    ...
    tbnConvert: TToolButton;
    ToolButton1: TToolButton;
    tbnTimeStamp: TToolButton;
    ...
    procedure tbnConvertClick(Sender: TObject);
    procedure tbnHeaderClick(Sender: TObject);
    procedure tbnTestClick(Sender: TObject);
    procedure tbnOpenClick(Sender: TObject);
    procedure tbnTimeStampClick(Sender: TObject);
...
begin
  ...
  begin
    ...
    begin
      StatBar( 1, 'Version: ' + IntToStr(ver1)+'.'+IntToStr(ver2));
    end;
...
begin
  ...
  if _irbacs64_getParam( ihnd, Ord( pavTimeStamp ), fValue ) then
    stgProps.Cells[1, 7] := FormatDateTime( 'dd.mm.yy hh:nn:ss',  fValue )
  else
    stgProps.Cells[1, 7] := '---';
  ...
  if _irbacs64_getParam( ihnd, Ord( pavSensorTemp ), fValue ) then
    stgADVals.Cells[1, 0] := FormatFloat( '0.###', fValue )
  else
    stgADVals.Cells[1, 0] := '---';

  if _irbacs64_getParam( ihnd, Ord( pavAD1 ), fValue ) then
    stgADVals.Cells[1, 1] := FormatFloat( '0.###', fValue )
  else
    stgADVals.Cells[1, 1] := '---';

  if _irbacs64_getParam( ihnd, Ord( pavDigVals ), fValue ) then
...
begin
  if _irbacs64_audioComment( FIrbacsHnd, 'audio.wav' ) then
    StatBar( 2, 'audioComment ok audio.wav' )
  else
    StatBar( 2, 'audioComment failed' );
end;
...
begin
  ...
  if _irbacs64_saveSingleFrame( FIrbAcsHnd, 'zzz_0.irb', nil )  then
    StatBar( 2, 'saveSingleframe ok zzz0.irb' )
  else
...
procedure TfrmTestLib.tbnConvertClick(Sender: TObject);
var
  iPixelSize, ii : Integer;
  kelvdat : Pointer;
  pDoub : PDouble;
  corrpars : TCorrPars;
begin
  iPixelSize := _irbacs64_readPixelData( FIrbacsHnd, nil, 2 );     //  Erfragen der Speicherreservierung
  Getmem( kelvdat, iPixelSize);
  iPixelSize := _irbacs64_readPixelData( FIrbacsHnd, kelvdat, 2 ); // Laden der Pixeldaten als Rohwerte
  ...
  iPixelSize := _irbacs64_convertPixelToKelvin( FIrbacsHnd, kelvdat, iPixelSize, @corrpars );
  StatBar( 2, 'convertPixelToKelvin: ' + IntToStr( iPixelSize ) );
end;

procedure TfrmTestLib.tbnHeaderClick(Sender: TObject);
begin
...
procedure TfrmTestLib.tbnTimeStampClick(Sender: TObject);
var
  aST : TDecodedTime;
begin
  aST := _irbacs64_getFrameTimeStamp( FIrbacsHnd );
  StatBar( 2, 'getFrameTimeStamp ' + IntToStr( aST.wDay ) + '.' + IntToStr( aST.wMonth ) + '.' + IntToStr( aST.wYear )
                             + ' ' + IntToStr( aST.wHour )+ ':' + IntToStr( aST.wMinute )+ ':' + IntToStr( aST.wSecond )
                             + ',' + IntToStr( aST.wMilliSec ) );
end;

procedure TfrmTestLib.tbnVisClick(Sender: TObject);
begin
  if _irbacs64_exportVisBitmap( FIrbacsHnd, 'vis.bmp' ) then
    StatBar( 2, 'exportVisBitmap ok vis.bmp' )
  else
    StatBar( 2, 'exportVisBitmap failed' );
end;
...
begin
  ...
  try
    ...
    on E : Exception do
    begin
     StatBar( 2, 'Error in OpenIrbfile (' + e.ClassName  + ' ' + e.Message + ')' );
    end;
  end;
end;
...
begin
  try
    ...
    begin
      Result := False;
      StatBar( 2, 'Error in CloseIrbFile (' + e.ClassName  + ' ' + e.Message + ')' );
    end;
  end;
end;
...
begin
  try
    iPixelSize := _irbacs64_readPixelData( FIrbacsHnd, nil, 3 );
    Getmem( kelvdat, iPixelSize);
    iPixelSize := _irbacs64_readPixelData( FIrbacsHnd, kelvdat, 3 );

    pSi := PSingle( kelvdat );
    MinVal := pSi^;
    MaxVal := pSi^;
    Inc( pSi );
    for ii := 1 to iPixelSize div sizeof(Single) - 1 do
    begin
      if pSi^ < MinVal then MinVal := pSi^;
      if pSi^ > MaxVal then MaxVal := pSi^;
      Inc( pSi );
    end;
    FreeMem( kelvdat );
